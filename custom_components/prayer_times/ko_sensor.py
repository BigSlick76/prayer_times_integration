...
# Chemin dynamique pour accéder au dossier 'data' à partir du répertoire de configuration
...
class IqamaDelaySensor(Entity):
    def __init__(self, city, prayer, delay):
        self.city = city
        self.prayer = prayer
        self.delay = delay
        self._state = delay
    ...
    @property
    def name(self):
        return f"{self.city}_delay_{self.prayer}"
...
def setup_platform(hass, config, add_entities, discovery_info=None):
    ...
    for city in cities:
        ...
        for prayer in ['Fajr', 'Dhuhr', 'Asr', 'Maghrib', 'Isha']:
            sensors.append(PrayerTimeSensor(city, prayer, prayer_times))         # Heure de prière
            sensors.append(IqamaTimeSensor(city, prayer, prayer_times, iqama_times))  # Heure d'iqama ajustée
            sensors.append(IqamaDelaySensor(city, prayer, iqama_times[prayer]))   # Délai d'iqama
